export default {
    settings: {
      ...
      conversationLog: {
        title: '对话记录',
        description: '异步记录用户请求与 AI 响应，便于后续查看与排查',
        enabled: '启用对话记录',
        enabledHint: '记录每一次模型调用；队列满时默认同步回写，避免记录丢失',
        storeRequest: '保存请求体',
        storeRequestHint: '完整保存用户输入与请求 payload',
        storeResponse: '保存响应体',
        storeResponseHint: '完整保存 AI 回复与响应 payload',
        workerCount: '写入 Worker 数',
        queueSize: '队列容量',
        taskTimeoutSeconds: '写入超时（秒）',
        overflowPolicy: '队列满策略',
        overflowPolicySync: '同步回退',
        overflowPolicyHint: '同步回退可避免丢记录；极端高负载下可能占用请求收尾时间',
        unlimitedBodyHint: '请求体和响应体会完整保存，不做字节截断',
        saved: '对话记录设置已保存',
    ...
    conversationLogs: {
      ...
      liveStreamTitle: '实时对话流',
      liveStreamHint: '仅展示开启后新写入的对话记录。',
      demoLiveStreamHint: '本地演示每 2.5 秒模拟一条进行中的对话；正式环境使用 SSE 实时推送。',
      startLiveStream: '开启实时流',
      startLiveDemo: '开启实时演示',
      stopLiveStream: '关闭实时流',
      inProgress: '进行中',
      liveConnecting: '正在连接…',
      liveConnected: '已连接',
      liveDisconnected: '已断开',
      liveEmpty: '等待新的对话记录…',
      latestUserMessage: '用户最新消息',
      search: '搜索',
      mode: '模式',
      searchPlaceholder: '搜索请求ID、响应ID、模型、用户、API 密钥或账号...',
      ...
      details: '对话详情',
      requestInfo: '请求信息',
      requestInfoHint: '点击查看用户、密钥、路由、用量和标识信息',
      requestBody: '用户请求',
      responseBody: 'AI 响应',
      ...
      timeline: {
        title: '对话执行时间线',
        collapseTimeline: '收起执行时间线',
        expandTimeline: '展开执行时间线',
        description: '按轮次查看上下文、模型回复，以及每次工具调用和返回结果。',
        searchPlaceholder: '搜索消息、调用、参数或结果',
        jumpToRound: '跳转轮次',
        searchResultCount: '命中 {count} 条',
        noSearchResults: '未找到匹配的对话内容',
        noSearchResultsHint: '请尝试其他关键词，或清空搜索以查看完整对话。',
        newestFirst: '倒序',
        oldestFirst: '顺序',
        round: '第 {index} 轮',
        context: '上下文',
