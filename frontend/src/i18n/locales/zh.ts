export default {
  ...
  admin: {
    ...
    settings: {
      ...
      conversationLog: {
        title: '对话记录',
        description: '异步记录用户请求与 AI 响应，开启后可在对话记录页检索',
        enabled: '启用对话记录',
        enabledHint: '开启后记录每次模型调用的请求与响应，不阻塞正常对话返回',
        storeRequest: '保存请求内容',
        storeRequestHint: '记录用户输入与请求体片段',
        storeResponse: '保存响应内容',
        storeResponseHint: '记录模型回复与响应体片段',
        workerCount: '写入线程数',
        queueSize: '队列长度',
        taskTimeoutSeconds: '写入超时（秒）',
        overflowPolicy: '队列满策略',
        overflowPolicySync: '同步兜底',
        overflowPolicyDrop: '丢弃新任务',
        overflowPolicyHint: '同步兜底可最大限度避免丢记录，但极端情况下会占用当前请求时间',
        maxRequestBytes: '请求最大字节数',
        maxResponseBytes: '响应最大字节数',
        bytesHint: '0 表示不限制字节数',
        unlimitedBodyHint: '请求与响应正文会完整保存，不再按字节数截断',
        saved: '对话记录设置保存成功',
