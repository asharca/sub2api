...
const { list, bulkAction, bulkAssign, listUsers, showError, showSuccess, showWarning } = vi.hoisted(() => ({
...
describe('subscription bulk operations', () => {
  ...
  it('assigns multiple users once and retries only failed users', async () => {
    listUsers.mockResolvedValue({
      items: [
        { id: 11, email: 'user11@example.com', status: 'active' },
        { id: 22, email: 'user22@example.com', status: 'active' }
      ],
      total: 2,
      pages: 1
    })
    ...
    await flushPromises()
    const form = wrapper.get('#bulk-assign-subscription-form')
    await form.get('[data-test="user-row-11"] input').setValue(true)
    await form.get('[data-test="user-row-22"] input').setValue(true)
    ...
    resolveAssign({
      ...
      reused_count: 0,
      failed_count: 1,
      subscriptions: [{ user_id: 11 }],
      errors: ['User 22: conflict'],
      statuses: { 11: 'created', 22: 'failed' }
    })
    await flushPromises()
    expect((form.get('[data-test="user-row-11"] input').element as HTMLInputElement).checked).toBe(false)
    expect((form.get('[data-test="user-row-22"] input').element as HTMLInputElement).checked).toBe(true)
    expect(form.get('[data-test="assignment-result"]').text()).toContain('User 22: conflict')
    bulkAssign.mockResolvedValueOnce({
      success_count: 1,
      created_count: 1,
      reused_count: 0,
      failed_count: 0,
      subscriptions: [{ user_id: 22 }],
      errors: [],
      statuses: { 22: 'created' }
    })
    await form.trigger('submit')
    ...
    expect(wrapper.find('#bulk-assign-subscription-form').exists()).toBe(false)
  })
})
