...
func (c *Config) Validate() error {
	...
	if c.Gateway.ConversationLog.Enabled {
		...
		switch strings.ToLower(strings.TrimSpace(c.Gateway.ConversationLog.OverflowPolicy)) {
		case UsageRecordOverflowPolicyDrop, UsageRecordOverflowPolicySync:
		default:
			return fmt.Errorf("gateway.conversation_log.overflow_policy must be one of: %s/%s",
				UsageRecordOverflowPolicyDrop, UsageRecordOverflowPolicySync)
		}
